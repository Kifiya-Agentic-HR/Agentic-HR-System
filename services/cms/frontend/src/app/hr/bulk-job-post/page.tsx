  ...
  const handleBulkUpload = async (values: FormSchemaType) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.failed_resumes && result.failed_resumes.length > 0) {
          toast.error(`Failed to process ${result.failed_resumes.length} resumes`);
        }

      } else {
        if (result.failed_resumes && result.failed_resumes.length > 0) {
          setFailedResumes(result.failed_resumes || []);
          toast.error(`Failed to process ${result.failed_resumes.length} resumes`);
        }
