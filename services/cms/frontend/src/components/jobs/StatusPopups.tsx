...
import { CheckCircle, XCircle, AlertTriangle, Clock, FileText, MessageSquareText, Zap } from 'lucide-react';
import { motion } from 'framer-motion';
import ProgressBar from '@/components/ui/progress-bar';
import { acceptApplication, rejectApplication, scheduleInterview } from '@/lib/api';
...
const StatusPopup = ({ 
  ...
  const hasInterviewScheduled = application.interview && 
  ...
  const renderScreeningContent = () => (
    <div className="space-y-6">
      ...
      {application.screening?.reasoning?.map((criterion, index) => (
        <div key={index} className="p-4 bg-white rounded-lg border border-[#364957]/20">
            ...
            {criterion.evidence.length > 0 ? (
              <ul className="list-disc pl-5 space-y-1">
                {criterion.evidence.map((item, i) => (
                  <li key={i} className="text-[#364957]/80">{item}</li>
                ))}
              </ul>
            ) : (
              <p className="text-[#364957]/60 italic">No supporting evidence found</p>
            )}
          </div>
          ...
          <div>
            ...
            {criterion.missing_elements.length > 0 ? (
              ...
              <p className="text-[#364957]/60 italic">All required elements present</p>
            )}
          </div>
        </div>
      ))}
      ...
      <div className="space-y-6">
       {application.interview.interview_status === 'completed' && application.interview.score && (
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {application.interview.interview_status === 'flagged' && application.interview.violations && (
            <div className="p-4 bg-red-50 rounded-lg border border-red-100">
              ...
              <div className="space-y-2">
                {application.interview.violations.split('\n').map((violation, i) => {
                  ...
                  return (
                    <div key={i} className={`flex items-start gap-2 text-sm ${isMajor ? 'text-red-600' : 'text-[#364957]/80'}`}>
                      <span className={`inline-block w-2 h-2 rounded-full mt-1 ${isMajor ? 'bg-red-500' : 'bg-[#364957]/50'}`} />
        ...
        {application.interview.interview_status === 'completed' && application.interview.skill_assessment && (
          <div className="p-4 bg-[#364957]/5 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4">
              {Object.entries(application.interview.skill_assessment).map(([skill, data]) => (
                <div key={skill} className="p-4 bg-white rounded-lg border border-[#364957]/20">
        ...
        <div className="p-4 bg-[#364957]/5 rounded-lg">
          ...
          <div className="space-y-4">
            {application.interview.interview_reasoning.split(/\d+\)\s+/).slice(1).map((section, index) => {
  ...
  const renderActionButtons = () => {
    ...
    return (
      <div className="mt-6 flex gap-4">
        ...
        {type === 'interview' && application.interview?.interview_status === 'completed' && (
          <button
            onClick={handleHire}
            disabled={isProcessing}
    ...
    >
      ...
      >
        <div className="p-6">
          ...
          {type === 'screening' ? renderScreeningContent() : renderInterviewContent()}
          {renderActionButtons()}
