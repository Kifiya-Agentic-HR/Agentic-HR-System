...
const StatusPopup = ({
        ...
        <div className="items-baseline gap-4 justify-between">
          {editMode ? (
            <div className="w-full p-5 bg-white rounded-lg border border-gray-100 shadow-sm">
              <div>
                <label htmlFor="score" className="block text-sm font-medium text-gray-600 mb-1">
                  Screening Score
                  <span className="text-xs text-gray-400 ml-1">(0-100 scale)</span>
                </label>
  ...
  {application.screening?.score && !isNaN(Number(application.screening.score))
    ...
    : 'N/A'}
      ...
      {!editMode && application.screening?.reasoning?.map((criterion, index) => (
      ...
      <div className="space-y-6">
        {application.interview.interview_status === 'completed' && application.interview.score && (
          <div className="p-4 bg-[#364957]/5 rounded-lg">
            ...
            <div className="text-3xl font-bold text-[#364957]">
  {application.interview?.score && !isNaN(Number(application.interview.score ))
